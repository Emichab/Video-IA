// ─── NeoFrame Video Engine v6 ───
// Uses /api/fal proxy to avoid CORS. No API keys in frontend code.
...
async function falPollResult(endpoint, requestId, onProgress, startPct, endPct, msg) {
  for (var i = 0; i < 120; i++) {
    ...
    try {
      ...
      var p = startPct + (endPct - startPct) * Math.min(1, i / 40);
      onProgress?.(Math.floor(p), msg + " (" + (status.status || "procesando") + ")");

      if (status.status === "COMPLETED") {
        var result = await falProxy("result", endpoint, null, requestId);
        return result;
      }
      if (status.status === "FAILED") {
        throw new Error("Generación fallida: " + (status.error || "error desconocido"));
      }
    } catch (e) {
      if (e.message.includes("fallida")) throw e;
      if (i === 119) throw e;
    }
  }
...
export async function generateVideo({ prompt, style, duration, ratio, withAudio, withSubtitles, subtitleStyle, onProgress }) {
  ...
  try {
    ...
    if (withAudio) {
      endpoint = "fal-ai/seedance/video/v1.5/pro/text-to-video";
      body = {
        prompt: prompt,
    ...
    if (withSubtitles && withAudio) {
      ...
      if (subData.request_id) {
        ...
        if (subResult.video && subResult.video.url) {
          videoUrl = subResult.video.url;
        }
    ...
    return { blob: blob, url: url, mimeType: blob.type || "video/mp4" };

  } catch (err) {
    console.warn("fal.ai error:", err.message);
    onProgress?.(10, "Error con API: " + err.message.substring(0, 50) + "... Usando modo Canvas");
...
// ══════════════════════════════════════
// CANVAS FALLBACK (free, always works)
// ══════════════════════════════════════
async function generateCanvasVideo({ prompt, style, duration, ratio, onProgress }) {
  ...
  try {
    ...
    drawFrame = new Function("ctx", "W", "H", "frame", "total", "t", ABSTRACT());
  }
...
export async function createClipsFromVideo({ file, clipCount, clipDuration, format, instructions, onProgress }) {
  ...
  var wh = sizes[format] || [720, 1280];
  var W = wh[0], H = wh[1], fps = 30;
  var clips = [];
